...
class PytestApiMockConfig(BaseConfig):
    ...
    # Allure 报告配置
    ALLURE_REPORT_DIR: str = Field(default=artifact_path("reports", "pytest_api", "allure"), description="Allure报告目录")
    ALLURE_HISTORY_DIR: str = Field(default=artifact_path("reports", "pytest_api", "history"), description="Allure历史记录目录")

    # 日志配置
    LOG_DIR: str = Field(default=artifact_path("temp", "logs", "pytest_api"), description="日志目录")
