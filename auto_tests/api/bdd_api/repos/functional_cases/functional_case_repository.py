...
class FunctionalCaseRepository:
    ...
    def claim_action(self, claim_id: int, role: str, action: str) -> httpx.Response:
        return self.request(
            ...
            json={"action": action, "comment": "AUTO_BDD"},
        )
    ...
    def mcp_initialize(self, role: str = "employee") -> dict:
        return self.runtime.mcp.initialize(self.run_id, self.runtime.token(role))
    ...
    def mcp_rpc(
        self, method: str, params: dict | None = None, role: str = "employee"
    ):
        return self.runtime.mcp.rpc(
            self.run_id, self.runtime.token(role), method, params
        )
    ...
    def mcp_rpc_with_session(
        ...
        return self.runtime.mcp.rpc_with_session(
            self.run_id, self.runtime.token(role), session_id, method, params
        )
    ...
    def mcp_cancellation_probe(
        ...
        return self.runtime.mcp.cancellation_probe(
            self.run_id, self.runtime.token(role), name, arguments
        )

    def mcp_batch(self, requests: list[dict], role: str = "employee"):
        return self.runtime.mcp.batch(
            self.run_id, self.runtime.token(role), requests
        )

    def mcp_act_claim(self, claim_id: int, role: str) -> dict:
        return self.runtime.act_claim_mcp(claim_id, role)
    ...
    def sse_events(self, **kwargs):
        return self.runtime.sse.events(
            self.run_id,
            self.runtime.token("employee"),
            **kwargs,
        )

    def websocket_session(self, role: str = "employee"):
        return self.runtime.websocket.session(self.run_id, self.runtime.token(role))
    ...
    @staticmethod
    def websocket_send_raw(socket, payload) -> None:
        socket.send(payload)

    @staticmethod
    def websocket_receive(socket, timeout: float = 5):
        return socket.recv(timeout=timeout)

    def websocket_rejected(self, *, missing_run: bool = False) -> bool:
        if missing_run:
            url = f"{self.runtime.websocket.url}?token=invalid"
        else:
            url = f"{self.runtime.websocket.url}?test_run_id={self.run_id}&token=invalid"
        try:
    ...
    def grpc_incomplete_metadata_code(self, *, missing_run: bool) -> str:
        return self.runtime.grpc.create_claim_without_complete_metadata(
            self.run_id, self.runtime.token("employee"), missing_run=missing_run
        )
    ...
    def grpc_versioned_contract(self, version: int) -> dict:
        return self.runtime.grpc.versioned_contract(version)

    def grpc_approve_claim(self, amount: float, reason: str) -> list[str]:
        client = self.runtime.grpc
        claim = client.create_claim(
            self.run_id, self.runtime.token("employee"), amount, reason
        )
        return [
            client.act_claim(
                self.run_id, self.runtime.token(role), claim.claim_id
            ).status
    ...
    def grpc_get_review(self, job_id: int):
        return self.runtime.grpc.get_review(
            self.run_id, self.runtime.token("employee"), job_id
        )
