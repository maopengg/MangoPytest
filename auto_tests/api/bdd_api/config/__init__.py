...
使用方式:
    from auto_tests.api.bdd_api.config import get_config, settings
    ...
    if test_env_json:
        try:
            ...
            for item in test_env_list:
                if item.get("project") == "bdd_api":
                    env_enum = item.get("test_environment")
                    env_map = {"DEV": "dev", "TEST": "test", "PRE": "pre", "PROD": "prod"}
                    env = env_map.get(env_enum, "")
                    if env:
                        return env
        except Exception:
            pass
    from auto_tests.api.bdd_api import DEFAULT_ENV
