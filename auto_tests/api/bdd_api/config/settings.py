...
class BddApiMockConfig(BaseConfig):
    ...
    MOCK_ADMIN_TOKEN: str = Field(
        ...
        description="仅用于自动清理隔离 Test Run 的 Mock 管理令牌",
    )
    ...
    # Allure 报告配置
    ALLURE_REPORT_DIR: str = Field(default=artifact_path("reports", "bdd_api", "allure"), description="Allure报告目录")
    ALLURE_HISTORY_DIR: str = Field(default=artifact_path("reports", "bdd_api", "history"), description="Allure历史记录目录")
    
    # 日志配置
    LOG_DIR: str = Field(default=artifact_path("temp", "logs", "bdd_api"), description="日志目录")
