...
class ApiMockConfig(BaseConfig):
    ...
    MOCK_ADMIN_TOKEN: str = Field(
        ...
        description="仅用于自动清理隔离 Test Run 的管理令牌",
    )
    ...
    # Allure 报告配置
    ALLURE_REPORT_DIR: str = Field(default=artifact_path("reports", "simple_api", "allure"), description="Allure报告目录")
    ALLURE_HISTORY_DIR: str = Field(default=artifact_path("reports", "simple_api", "history"), description="Allure历史记录目录")
    
    # 日志配置
    LOG_DIR: str = Field(default=artifact_path("temp", "logs", "simple_api"), description="日志目录")
...
class DevConfig(ApiMockConfig):
    ...
    model_config = ConfigDict(env_file=".env.dev", env_file_encoding="utf-8", extra="allow")
...
class ProdConfig(ApiMockConfig):
    ...
    model_config = ConfigDict(env_file=".env.prod", env_file_encoding="utf-8", extra="allow")
...
class TestConfig(ProdConfig):
    ...
    model_config = ConfigDict(env_file=".env.test", env_file_encoding="utf-8", extra="allow")
...
class PreConfig(ProdConfig):
    ...
    model_config = ConfigDict(env_file=".env.pre", env_file_encoding="utf-8", extra="allow")
