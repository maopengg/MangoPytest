"""Mango Mock Repository 共用的 Test Run 与认证上下文。"""
...
class RepositoryContext:
    ...
    def ensure_run(self) -> None:
        ...
        result = self.http.create_run(
            f"{self.run_name_prefix}_{uuid.uuid4().hex[:10]}", ttl_minutes=5
        )
        if result.status_code != 201:
            raise AssertionError(result.body)
        self.run_id = result.data["id"]
        self.cleanup_token = result.data["cleanup_token"]
        log.debug(f"{self.run_name_prefix} 创建 Test Run: {self.run_id}")
            ...
            if self.run_id and not self.deleted:
                ...
                if result.status_code in {200, 404}:
                    ...
                    raise AssertionError(
                        f"{self.run_name_prefix} Test Run 清理失败: {result.body}"
                    )
        finally:
            self.http.close()
