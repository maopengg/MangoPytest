...
from auto_tests.demo_project.data_factory.entities.user_pydantic import UserEntity
...
class AuthBuilder(PydanticBuilder[UserEntity]):
    ...
    def register(self, entity: UserEntity) -> Optional[UserEntity]:
        ...
        result = demo_project.auth.api_register(**payload)
    ...
    def _delete_entity(self, entity: UserEntity) -> None:
        ...
        if entity.id:
            # 确保设置了 token
            if self.token:
                demo_project.auth.set_token(self.token)
            demo_project.user.delete_user(entity.id)
