...
class AuthSpec(BaseFactory):
    ...
    token = factory.Sequence(lambda n: f"AUTO-token-{n:06d}-{factory.Faker('uuid4').generate()}")
    ...
    class Params:
        ...
        expired = factory.Trait(
            expires_at=factory.LazyAttribute(lambda _: datetime.now() - timedelta(hours=1))
        )
