...
class TestSubmitData(UnitTest):
    ...
    def test_submit_data_success(self, authenticated_client):
        """测试正常提交数据"""
        bdd_api_mock.data.set_token(authenticated_client.token)
        result = bdd_api_mock.data.submit_data(name="test_metric", value=100)
    ...
    def test_submit_data_different_values(self, authenticated_client):
        ...
        # 提交0
        result1 = bdd_api_mock.data.submit_data(name="zero_value", value=0)
        assert result1.get("code") == 200
        assert result1.get("data", {}).get("value") == 0

        # 提交大数值
        result2 = bdd_api_mock.data.submit_data(name="large_value", value=999999)
        assert result2.get("code") == 200
        assert result2.get("data", {}).get("value") == 999999

        # 提交负数值 - API不允许负数，应该返回错误
        result3 = bdd_api_mock.data.submit_data(name="negative_value", value=-100)
    ...
    def test_submit_data_special_names(self, authenticated_client):
        ...
        # 中文名称
        result1 = bdd_api_mock.data.submit_data(name="中文名称", value=100)
        assert result1.get("code") == 200

        # 包含空格
        result2 = bdd_api_mock.data.submit_data(name="name with spaces", value=200)
        assert result2.get("code") == 200

        # 包含特殊字符
        result3 = bdd_api_mock.data.submit_data(
    ...
    def test_submit_data_empty_value(self, authenticated_client):
        ...
        bdd_api_mock.data.set_token(authenticated_client.token)
        # 使用0代替None，因为API期望整数
        result = bdd_api_mock.data.submit_data(name="test", value=0)
