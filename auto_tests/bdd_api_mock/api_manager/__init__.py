...
# 统一的API出口
from auto_tests.bdd_api_mock.config import load_settings
...
class DemoProjectAPI:
    """
    统一的API出口类
    使用方式: bdd_api_mock.auth.api_login(data)

    配置host：
        bdd_api_mock.set_host("http://localhost:8003")
        bdd_api_mock.sync_host_from_env("pre")

    配置token：
        bdd_api_mock.set_token("your_token_here")
    """
    ...
    def sync_host_from_env(self, env: str):
        ...
        """
        from auto_tests.bdd_api_mock.config import sync_host_from_env
