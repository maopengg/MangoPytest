...
@given(parsers.parse('假如存在"{entity_name}":'), target_fixture="created_entity")
def create_entity_step_with_prefix(entity_name: str, docstring):
    """创建实体步骤（带假如前缀）"""
    return create_entity_step(entity_name, docstring)


@given(parsers.parse('存在"{entity_name}"'), target_fixture="created_entity")
def create_entity_step_simple(entity_name: str):
    """创建实体步骤（无参数）
...
@given(parsers.parse('假如存在"{entity_name}"'), target_fixture="created_entity")
def create_entity_step_simple_with_prefix(entity_name: str):
    """创建实体步骤（无参数，带假如前缀）"""
    return create_entity_step_simple(entity_name)
...
def verify_entity_in_db(entity_name: str, created_entity):
    """验证数据库中存在实体"""
