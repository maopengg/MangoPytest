...
def _get_default_env() -> str:
    ...
    从 project_registry.py 读取 MOCK_API 项目的环境配置
    如果失败则返回空字符串
    """
    try:
        ...
        env_enum = get_project_environment(ProjectEnum.SIMPLE_API)
        env_map = {
...
def get_config(env: str = None) -> Union[DevConfig, TestConfig, PreConfig, ProdConfig]:
    ...
    Raises:
        ValueError: 如果环境未设置且无法从 project_registry.py 读取
    ...
    if env is None:
        ...
        env = os.getenv("ENV")
        if not env:
            # 尝试从 project_registry.py 读取
            env = _get_default_env()
            if not env:
                raise ValueError(
                    "未设置测试环境。请通过以下方式之一设置：\n"
                    "1. 设置环境变量：export ENV=dev 或 set ENV=dev\n"
                    "2. 在 project_registry.py 中配置 PROJECT_ENVIRONMENT"
                )
