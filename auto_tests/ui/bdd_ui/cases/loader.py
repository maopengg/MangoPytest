...
def _validate_unique(cases, label: str) -> None:
    if not cases:
        raise ValueError(f"bdd_ui {label}用例不能为空")
    case_ids = [case.case_id for case in cases]
    if len(case_ids) != len(set(case_ids)):
        raise ValueError(f"bdd_ui {label}用例 ID 存在重复")
...
def _validate_element_references() -> None:
    ...
    if missing:
        raise ValueError(f"bdd_ui 用例引用了不存在的元素：{sorted(missing)}")
    inventory = {case.element_id for case in INVENTORY_CASES}
    if inventory != known:
        raise ValueError("bdd_ui 元素契约清单与本地 Excel 不一致")
