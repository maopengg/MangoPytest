"""读取并校验 bdd_ui 自己维护的本地 Excel 元素工作簿。"""
...
def _validate_records(records: tuple[dict[str, Any], ...]) -> None:
    if not records:
        raise ValueError("bdd_ui 本地元素 Excel 不能为空")
    names = [str(record["元素名称"]).strip() for record in records]
    ids = [int(record["ID"]) for record in records]
    if len(names) != len(set(names)):
        raise ValueError("bdd_ui 本地元素 Excel 存在重复元素名称")
    if len(ids) != len(set(ids)):
        raise ValueError("bdd_ui 本地元素 Excel 存在重复元素 ID")
