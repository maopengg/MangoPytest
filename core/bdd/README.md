...
```python
# auto_tests/bdd_api_mock/conftest.py
...
```python
# auto_tests/qfei_contract_api/conftest.py
...
@pytest.fixture(scope="session")
def entity_factory_map():
    from auto_tests.qfei_contract_api.data_factory.specs import (
        ContractSpec, AttributeSpec,
    )
    return {
        "合同": ContractSpec,
        "属性": AttributeSpec,
