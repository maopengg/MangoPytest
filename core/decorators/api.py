...
def case_data(
    ...
    def decorator(func):
        ...
        if case_id:
            ...
            test_case_list = SourcesData.get_api_test_case(
                is_dict=False, name=case_name
            )
        else:
        ...
        def wrapper(self, test_case, **fixture_kwargs):
            ...
            allure.attach(
                test_case_model.model_dump_json(),
                "用例数据",
...
def _prepare_fixture_data(
    fixture_name: str, fixture_kwargs: dict, cleanup_ctx: CleanupContext
):
    """
    根据名称准备fixture数据
    支持两种模式：
    ...
    raise PytestAutoTestError(
        f"未找到fixture: {fixture_name}，请确保在测试类中定义对应的fixture"
    )
