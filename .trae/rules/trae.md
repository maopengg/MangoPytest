...
```python
# 导入异常类
from core.exceptions import ApiError, UiError, ERROR_MSG_0001, ERROR_MSG_0400
# 抛出异常
raise ApiError(*ERROR_MSG_0400)
```
...
2. **业务代码** (`auto_tests/api/pytest_api/`) 只使用现有组件，不创建新组件
