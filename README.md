...
mango_pytest 是一个集 **API 自动化测试** 于一体的测试平台，采用多种架构模式满足不同场景的测试需求。项目包含三个独立的 API 测试子项目，分别采用不同的设计模式。
...
| 项目 | 优势 | 劣势 | 推荐指数 |
|------|------|------|---------|
| **api_mock** | ✅ 学习成本低，快速上手<br>✅ 代码简洁直观<br>✅ 维护简单，适合小团队<br>✅ 与 UI 页面对象模式一致 | ❌ 数据准备逻辑分散<br>❌ 复杂依赖场景代码冗余<br>❌ 缺少数据自动清理机制 | ⭐⭐⭐⭐⭐<br>（推荐新手） |
