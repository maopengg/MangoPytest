...
# 数据模型
from models import ApiDataModel, RequestModel, ResponseModel

# 枚举
from enums import MethodEnum, ClientEnum

# 异常
from exceptions import ApiError, UiError
from exceptions.error_msg import ERROR_MSG_0400
```
...
# 导入错误消息
from exceptions.error_msg import ERROR_MSG_0001, ERROR_MSG_0400
...
- ❌ 在 exceptions/__init__.py 中导出错误消息
...
- [ ] 是否需要新的数据模型？→ 如果是，添加到 `models/`
- [ ] 是否需要新的枚举？→ 如果是，添加到 `enums/`
- [ ] 是否需要新的异常？→ 使用现有的异常类，在 `error_msg.py` 中添加错误消息
...
| models/ | `功能_model.py` | `api_model.py`, `ui_model.py` |
| enums/ | `功能_enum.py` | `api_enum.py`, `ui_enum.py` |
...
- ✅ 在 `auto_test/demo_project/` 中实现业务逻辑
- ✅ 通过继承和组合使用现有组件
...
1. **核心模块** (`core/`, `models/`, `enums/`, `exceptions/`) 只存放**全局通用**的组件
2. **业务代码** (`auto_test/demo_project/`) 只使用现有组件，不创建新组件
3. **统一导入** 从模块的 `__init__.py` 导入，不从具体文件导入
4. **异常处理** 使用现有的 7 个异常类 + `error_msg.py` 中的错误消息
