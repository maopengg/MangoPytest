# MangoPytest pytest_api - 数据工厂与自动化测试框架演示
...
class OrderBuilder:
    def create_order(self, entity: OrderEntity) -> OrderEntity:
        ...
        result = pytest_api.order.create_order(payload)
...
```
auto_test/pytest_api/
├── api_manager/          # 【L1: 接口层】API 管理层 - 统一接口封装与协议处理
│   ├── __init__.py       # 统一出口 pytest_api
...
```python
from auto_tests.api.pytest_api.api_manager import pytest_api

# 设置认证 token
pytest_api.auth.set_token("your_token")

# 调用用户 API - L1 只接收 Dict
result = pytest_api.user.get_all_users()

# 调用订单 API - L1 只接收 Dict
result = pytest_api.order.create_order({
    "product_id": 1,
    "quantity": 2,
    "user_id": 1
})
...
```python
from auto_tests.api.pytest_api.data_factory.entities import OrderEntity
...
```python
from auto_tests.api.pytest_api.data_factory.builders import OrderBuilder
from auto_tests.api.pytest_api.data_factory.entities import OrderEntity
...
```python
from auto_tests.api.pytest_api.data_factory.scenarios import CreateOrderScenario
...
```python
from auto_tests.api.pytest_api.fixtures.infra import (
    api_client,      # API 客户端
    test_token,      # 测试 token
    test_context,    # 测试上下文
)
...
```python
from auto_tests.api.pytest_api.fixtures.entities import (
    admin_user,      # 管理员用户
    locked_user,     # 已锁定用户
    employee_user,   # 员工用户
    default_org,     # 默认组织
...
```python
from auto_tests.api.pytest_api.fixtures.builders import (
    # D模块（基础层）
    org_builder,     # 组织构造器
    user_builder,    # 用户构造器
    # C模块（预算层）
...
```python
from auto_tests.api.pytest_api.fixtures.scenarios import (
...
```python
from auto_tests.api.pytest_api.test_cases.decorators import case_data
from auto_tests.api.pytest_api.data_factory.scenarios import LoginScenario
...
class TestOrderIntegration(UnitTest):
    ...
    def test_create_query_cancel_flow(self, api_client):
        ...
        # 2. 查询订单（直接调用 L1）
        queried = pytest_api.order.get_order(created.order_id)
        
        # 3. 取消订单
        cancelled = pytest_api.order.cancel_order(created.order_id)
...
```bash
# 运行所有测试
pytest auto_tests/api/pytest_api/test_cases/

# 运行特定模块
pytest auto_tests/api/pytest_api/test_cases/test_order.py

# 运行特定测试
pytest auto_tests/api/pytest_api/test_cases/test_order.py::TestCreateOrder::test_create_order_success

# 生成 Allure 报告
pytest auto_tests/api/pytest_api/test_cases/ --alluredir=./allure-results
...
class OrderBuilder:
    def create_order(self, entity: OrderEntity):
        ...
        return pytest_api.order.create_order(payload)
