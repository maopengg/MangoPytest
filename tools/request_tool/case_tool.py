...
from config.settings import PRINT_EXECUTION_RESULTS
        ...
        for i in case_after:
            if i.get('type') == AfterHandleEnum.sql.value:
                for sql in i.get('after_handle'):
                    ...
                    #     ERROR.logger.error(f'后置操作失败，请检查：{e}，数据：{case_after}{project}')


if __name__ == '__main__':
    response_dic = {"status": 0, "message": None, "data": None}
    ass = [{"ass_type": 0, "sql": None,
            "expect_list": [{"actual": "$.data", "ass_method": "value_is_null", "expect": None},
                            {"actual": "$.message", "ass_method": "value_is_null", "expect": None},
                            {"actual": "$.status", "ass_method": "is_equal_to",
                             "expect": 0}]}]
    for e in ass:
        CaseTool().response_ass(response_dic, AssListModel(**e))
    ...
    sql1 = [
        {
            "ass_type": 1,
            "sql": "select `status` from sync_log ORDER BY create_time DESC LIMIT 1;",
            "expect_list": [
                {
                    "actual": "status",
                    "ass_method": "is_equal_to",
                    "expect": 1
                }
            ]
        }
    ]
    CaseTool().sql_ass(AssListModel(**sql1[0]), TEST_AIGC_MYSQL)
