...
from auto_test.demo_project.data_factory.builders.finance_approval import FinanceApprovalBuilder


@pytest.fixture
def finance_approval_builder(api_client) -> FinanceApprovalBuilder:
    """
    财务审批Builder Fixture
    ...
    """
    builder = FinanceApprovalBuilder(token=api_client.token)
    yield builder
    # 清理创建的数据
    builder.cleanup()
...
@pytest.fixture
def finance_approved_reimbursement(reimbursement_builder, dept_approval_builder,
                                   finance_approval_builder) -> dict:
    """
    已通过财务审批的报销申请Fixture
    提供D级+C级+B级完整数据
    """
...
@pytest.fixture
def finance_rejected_reimbursement(reimbursement_builder, dept_approval_builder,
                                   finance_approval_builder) -> dict:
    """
    被财务拒绝的报销申请Fixture
    提供D级+C级+B级(拒绝)完整数据
