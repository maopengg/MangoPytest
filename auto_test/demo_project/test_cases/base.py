    ...
    def expect_field(self, field_path: str, expected_value: Any) -> 'TestContext':
        ...
        if current:
            ...
            for part in field_path.split("."):
                value = getattr(value, part, None) if hasattr(value, part) else value.get(part) if isinstance(value,
                                                                                                              dict) else None
...
class TestLayer(ABC):
    ...
    def assert_success(self, result: Dict, message: str = ""):
        """断言成功"""
        assert result.get("code") == 200, f"期望成功，实际: {result.get('code')} - {result.get('message', message)}"

    def assert_failure(self, result: Dict, expected_code: int = None, message: str = ""):
        """断言失败"""
        if expected_code:
            assert result.get("code") == expected_code, f"期望错误码 {expected_code}，实际: {result.get('code')}"
        else:
    ...
    def assert_field_equals(self, result: Dict, field_path: str, expected_value: Any):
        ...
        assert value == expected_value, f"字段 {field_path} 期望 {expected_value}，实际 {value}"
