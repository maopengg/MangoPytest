...
class TestFullApprovalWorkflow(E2ETest):
    ...
    def test_full_approval_workflow_success(self, api_client, approval_scenarios, test_context):
        """测试完整的4级审批流程 - 全部通过"""
    ...
    def test_dept_rejection_workflow(self, api_client, approval_scenarios, test_context):
        """测试部门审批拒绝流程"""
    ...
    def test_finance_rejection_workflow(self, api_client, approval_scenarios, test_context):
        """测试财务审批拒绝流程"""
...
class TestApprovalDependencies(IntegrationTest):
    ...
    def test_get_full_workflow(self, authenticated_client, fully_approved_reimbursement, 
...
class TestApprovalDependencyValidation(IntegrationTest):
    ...
    def test_cannot_skip_dept_approval(self, authenticated_client, pending_reimbursement,
    ...
    def test_cannot_skip_finance_approval(self, authenticated_client, 
                                          dept_approved_reimbursement, ceo_id):
        """测试不能跳过财务审批直接进行总经理审批"""
        # 尝试直接进行总经理审批（没有财务审批）
    ...
    def test_dept_rejected_cannot_proceed(self, authenticated_client, 
                                          dept_rejected_reimbursement, finance_manager_id):
        """测试部门拒绝后不能进行财务审批"""
    ...
    def test_finance_rejected_cannot_proceed(self, authenticated_client,
                                             finance_rejected_reimbursement, ceo_id):
        """测试财务拒绝后不能进行总经理审批"""
...
class TestApprovalParameterized(E2ETest):
    ...
    def test_approval_with_different_amounts(self, approval_scenarios, test_context,
    ...
    def test_approval_scenario_matrix(self, full_approval_workflow, dept_rejected_workflow,
                                      finance_rejected_workflow, ceo_rejected_workflow):
        """测试审批流程场景矩阵"""
        # 验证所有场景fixture都正常工作
        assert full_approval_workflow["status"] == "fully_approved"
        assert dept_rejected_workflow["status"] == "dept_rejected"
...
class TestApprovalPerformance(E2ETest):
    ...
    def test_full_approval_performance(self, approval_scenarios):
        ...
        assert workflow["status"] == "fully_approved"
        # 验证执行时间在合理范围内（例如5秒内）
        assert duration < 5.0, f"审批流程执行时间过长: {duration}秒"
